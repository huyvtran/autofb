...
class unfriends extends MX_Controller {
	...
	public function index(){
		$data = array(
			...
			"save"    => $this->model->fetch("*", SAVE, "status = 1 AND category = 'unfriends'".getDatabyUser()),
			"categories" => $this->model->fetch("*", CATEGORIES, "category = 'message'".getDatabyUser())
		);
		$this->template->title(l('Auto unfriends'));
	...
	public function ajax_page()
    {
        ...
        $data = array();
            ...
            foreach($books->result() as $r) {
                $avatar = "http://graph.facebook.com/".$r->pid."/picture?type=square";
            foreach($account as $g) {
                ...
                if ($g->id == $r->account_id)
                   $data[] = array(
                        ...
                        $r->name,
                        $r->type,
                        '<img src="'.$avatar.'"> <a href="https://facebook.com/'.$r->pid.'" target="_blank"><i class="fa fa-link" aria-hidden="true"></i> Xem ngay</a>',
                        ""
            ...
            foreach($account as $g) {
                $data[] = array(
                    ...
                    $g->fullname,
                    "profile",
                    '<img src="'.$avatar.'"> <a href="https://facebook.com/'.$g->fid.'" target="_blank"><i class="fa fa-link" aria-hidden="true"></i> Xem ngay</a>',
                    ""
			...
			if(!empty($account)){
				...
				if($response->st == "success"){
					ms(array(
						...
						"txt"   => "<span class='col-green'>".$response->txt." <a href='https://facebook.com/".$group[3]."' target='_blank'><i class='col-light-blue fa fa-external-link-square' aria-hidden='true'></i></a></span>"
					));
				}else{
					ms(array(
						"st"    => "error",
