                    ...
                    <div class="tab-content pt15">
                        <div role="tabpanel" class="tab-pane fade active in" id="home">
                            <div class="row">
                                <div class="col-sm-12 mb0">
                                    ...
                                    <div class="form-group">
                                        <div class="form-line">
                                            <input type="text" class="form-control" name="phone" value="<?=!empty($result)?$result->pid:""?>">
                                        </div>
                                    </div>
                                    <b><?=l('Time zone')?></b>
                                    <div class="form-group">
                                        <select name="timezone" class="form-control">
											<option value ="<?=tz_list()[238]['zone']?>"> <?=tz_list()[238]['diff_from_GMT'] . ' - ' . tz_list()[238]['zone'] ?> </option>
                       ...
                       <!-- <div role="tabpanel" class="tab-pane" id="profile"> -->
                            ...
                            <div class="form-group">
								...
								<?php  if ($result_agency->admin == 1  || $result_agency->package_id == 9 || $result_agency->package_id == 8 || $result_agency->package_id == 6) {  ?>	
								
								<option value='<?=$package[2]->permission?>|<?=$package[2]->id?>'><?=$package[2]->name?></option> 
								
								<?php } if ($result_agency->admin == 1  || $result_agency->package_id == 9 || $result_agency->package_id == 8 ) {  ?>	
								
                                 <option class="open-agency" value='<?=$package[5]->permission?>|<?=$package[5]->id?>'><?=$package[5]->name?></option>
                                
								<?php }  if ($result_agency->admin == 1  || $result_agency->package_id == 9 ) {  ?>	
								 <option value='<?=$package[7]->permission?>|<?=$package[7]->id?>'><?=$package[7]->name?></option>
								
								<?php }  if ($result_agency->admin == 1) {  ?>	
								 <option value='<?=$package[8]->permission?>|<?=$package[8]->id?>'><?=$package[8]->name?></option>								
								 <?php } ?>
                                </select>
                            </div>
                        <!--    <b><?=l('Số tài khoản')?></b> -->
                        ...
                        <!--    <b><?=l('Số lượng friends')?> (<span class="col-red">*</span>)</b>  -->
							...
							<b><?=l('Số tháng sử dụng của đại lý')?></b> 
							<div class="form-group box-open-agency">
								<div class="form-line">
									<input type="text" class="form-control" name="time_for_agency" value="<?=!empty($result)?$result->agency:""?>">
								</div>
							</div>
