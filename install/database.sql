...
ALTER TABLE `facebook_schedules` ADD `unique_content` INT(1) NULL DEFAULT '0' AFTER `caption`, ADD `unique_link` INT(1) NULL DEFAULT '0' AFTER `unique_content`;
ALTER TABLE `payment` ADD `pagseguro_email` TEXT NULL AFTER `paypal_email`, ADD `pagseguro_token` TEXT NULL AFTER `pagseguro_email`, ADD `stripe_email` TEXT NULL AFTER `pagseguro_token`, ADD `stripe_pk` TEXT NULL AFTER `stripe_email`, ADD `stripe_sk` TEXT NULL AFTER `stripe_pk`, ADD `symbol` TEXT NULL AFTER `currency`;
ALTER TABLE `payment_history` ADD `type` VARCHAR(255) NULL AFTER `id`;
ALTER TABLE `payment_history` ADD `feeAmount` FLOAT NULL AFTER `mc_gross`;
ALTER TABLE `payment_history` ADD `netAmount` FLOAT NULL AFTER `feeAmount`;
ALTER TABLE `facebook_groups` CHANGE `name` `name` TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL;
...
--
-- Máy chủ: localhost
-- Thời gian đã tạo: Th8 28, 2017 lúc 02:32 AM
